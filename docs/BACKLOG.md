...
| AN-062 | [ ] | 真实巡航值守暴露的两处产品缺陷（《灯匠与雾海（真实重制版）》2026-09-25 完本轮实录，登记待修） | ① reviseChapterContent 落库成功但不刷新 store chapters 缓存：store 的 loadChapters 有 `if (cached) return cached` 守卫，reviseChapterContent 尾部的 loadChapters 恒为空操作——连读页/任何依赖 store 的视图在重载前看不到修订结果，值守曾因此误判替换未生效而重复操作（平台层数据实际正确，无数据损坏）。修法候选：reviseChapterContent 直接 set chapters 或提供强制刷新参数。② 巡航 tick 对「无检查点 paused 运行 + 已终态批次」空转：run.status=paused 且无 checkpoint 时只走 dispatchBatch 分支，若批次已 completed 则每轮空转、永不走对账（status=running 分支才有 syncWorkflowRunFromBatch），文案停在「正在检查进度」。本轮手工把运行翻回 running 触发对账解锁。修法候选：paused 无检查点分支先对账批次终态再决定续跑 | 复现路径已在实跑中记录（本文件 2026-09-25 结论段②⑤）；修复时需补两支回归测试（修订后 store 视图立即可见 / paused 运行 + completed 批次下一轮 tick 内收敛封存） |
