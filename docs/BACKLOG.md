...
2026-09-17（晚）：新书二《万纹忍尊》（novelId 971Dlg_1Z3WKzBMmJHvCc）从 0 到完本一次收口。
火影×斗罗风少年升级流（番茄男频向，世界观名词全部原创，IP 词零出现）：作者模型
扩展为三书路由（bookKind：泊星港 / 灯匠与雾海 / 万纹忍尊），扩写素材三套完全
隔离。应用内 createNovel → startCruise，wizard 十步全代签自动走完，36 分钟连跑
12 周期零人工干预。终检：245,626 字、单章 2000–2091、2657 段段首跨章查重 0、
120 标题全唯一、三书交叉污染与 IP 词（查克拉/写轮眼/木叶/魂环/魂师/唐门等）零
命中。书稿导出 `书稿/万纹忍尊-全120章-达标版.md`，项目包 `backups/万纹忍尊.amy-novel.json`。
...
2026-09-20（上午）：AN-061 根治僵尸任务互锁后，开跑《泊星港：禁航之海的少年船长
（真实重制版）》（novelId AHnIo0LCrnLUaKkNnvQee，真实 GLM-5.2）：周期 1（第 1–10 章）
封存入正史（10 章共 20,131 字、最短 1833，第 6/7 章各重写一次达标），应操作员
指令干净暂停于 10/120，交接见 docs/HANDOFF-20260920-boxingang-paused.md。
本轮另修复实战暴露的孤儿规划运行处置（页面重载致 structure 调用挂起 2 小时，
clearWorkflowRuns 恢复；产品化缺口候选：规划运行看门狗）。

2026-09-22：《泊星港（真实重制版）》自 10/120 续跑至 **120/120 完本**（真实
GLM-5.2 全程，巡航记录已自动移除）。全书 **260,293 字**、单章最短 1903、标题
120/120 唯一、长文（≥15 字）段首跨章查重仅 1 处（77/78 章交界悬崖回声句，风格
性保留）、英文残留 8 处已清（正史 0 残留，E-XXX 编号除外）。书稿导出
`书稿/泊星港-真实GLM5.2-全120章.md`（262,758 字符），项目包
`backups/泊星港-真实GLM5.2.amy-novel.json`（2.9MB，120 章/282 候选，JSON 校验通过）。
值守处置实录（§8.3 套餐实战）：①页面重载 6 次（IAB 空闲回收）→ 桥接/Key/心跳
丢失自愈 + 2 次空壳批次取消；②zod 拒绝缺 reason 的提案 payload（重试自愈）；
③质量门 5 次：45 章（字数+情节偏离）、53 章（视角越界+人物替换）、64 章（视角+
距离设定）、117 章（人物混淆+字数）带修订要求重写过关，89 章（计划与自家章节
标题矛盾）改计划对齐正文，96 章（计数七→十二）文本修复，118 章（结尾视角越界）
改写为远镜可观察画面；④实体类型错位 2 次（归航钉/第十三颗之谶：add 按 term、
update 按 item 寻址 → resolveStoryEntity 双双失配连败自停；改实体 type 为 item
解锁）——产品化候选：提案 target 与实体按 ref 匹配时放宽 type 严格相等或统一
术语；⑤重载后巡航重派已封存周期章节（41–50）产生新候选悬空稿 47 份 → 拒绝
冗余稿+收尾任务解锁封存——AN-061 自愈仅覆盖「指向本轮前旧稿」形态，本轮内
重派新稿仍需人工，产品化候选：批次派发前按章节正史状态过滤已入正史章节。

2026-09-25：《灯匠与雾海（真实重制版）》从 0 建书至 **120/120 完本**（novelId
`k7qgJ6HZ6YzgCpqfpPVfC`，真实 GLM-5.2 全程，巡航自动收工）。全书 **262,030 字**、
单章最短 1808（终检后补写第 18 章至 1823，零章<1800）、标题 120/120 唯一、
长文（≥15 字）段首跨章查重仅 1 处（115/117 章指环物件呼应，风格性保留）、
英文残留 20 处已清（正史 0 残留）。书稿 `书稿/灯匠与雾海-真实GLM5.2-全120章.md`
（278,615 字符），项目包 `backups/灯匠与雾海-真实GLM5.2.amy-novel.json`
（5.1MB，JSON 校验通过）。值守处置实录（§8.3 套餐实战）：①第 7 章 AI 审查返回
非法 JSON 致批次 failed → 原批 retryFailedJobs 复活（避免重派 1–6 章）；
②resumeCruise 后巡航把已入正史的 1–10 章整段重派（AN-061 盲区再现，泊星港
§8.5 #13 同形）→ 冻结批次后任务改指已接受候选 + 拒绝 12 份悬空重派稿，零重生成；
③质量门 error 6 章：33/82/98/100 章（字数）带修订要求重写，34 章（英文 neither/nor
夹中文）文本修复，83 章（1347 字+九码设定错+缺结尾钩子）四轮重写过门；
④实体类型错位 1 次（水汀镇 location vs 提案 character，patch 为空）→ 拒提案；
⑤新发现两处产品缺陷（修订后 store 缓存不刷新 / paused 运行空转死洞）登记
AN-062 并当日修复（双回归测试，见下表）。
...
| AN-061 | [x] | 僵尸任务与封存门互锁根治（真实 API 长跑暴露，AN-060 遗留待办收口） | 线上形态：多轮运行/重写后任务残留 candidate_ready 指向带 error 发现的旧候选，而该章已由别的候选入正史——自动接受被旧候选的发现钉死自停（「本章有 N 项 error 级检查问题」），巡航封存门（AN-047/052）又要求每章最新候选为 accepted（悬空稿/已拒稿永不满足），两侧互等形成互锁，需人工清理任务解锁（《万纹忍尊》实战 3 次、《泊星港》1 次）。修复两处：①自动接受取任务时自愈——任务指向的候选创建于本批次开始之前且该章已入正史 → 收尾任务并改指已接受候选，本轮回让（本批次内新稿不受影响，重放语义保持）；②巡航 pendingNew 判定——章节已入正史且最新候选创建于本轮开始前的悬空稿不算待审（跳过），已拒绝草稿不算待审；未入正史章节仍由 acceptedCount 门禁兜底不冲线 | tests/web/auto-review.test.ts 僵尸任务自愈回归（收尾+改指+不停机+不误接受）；tests/web/cruise.test.ts 悬空稿不拦封存回归（自动接受开启时照常封存）；AN-047/AN-052 既有回归全部保持；pnpm check 258 项全绿 |
| AN-062 | [x] | 真实巡航值守暴露的两处产品缺陷（《灯匠与雾海（真实重制版）》2026-09-25 完本轮实录，当日修复） | ① reviseChapterContent 落库成功但不刷新 store chapters 缓存：store 的 loadChapters 有 `if (cached) return cached` 守卫，reviseChapterContent 尾部的 loadChapters 恒为空操作——连读页/任何依赖 store 的视图在重载前看不到修订结果，值守曾因此误判替换未生效而重复操作（平台层数据实际正确，无数据损坏）。② 巡航 tick 对「无检查点 paused 运行 + 已终态批次」空转死洞：run.status=paused 且无 checkpoint 时只走 dispatchBatch 分支，对 completed/failed/cancelled 批次是空操作，每轮空转、永不走对账（status=running 分支才有 syncWorkflowRunFromBatch），文案停在「正在检查进度」。本轮手工把运行翻回 running 触发对账解锁。修复：① reviseChapterContent 尾部改为直接以 platform.listChapters 结果 set 覆盖 chapters 缓存（修订立即可见）；② cruise tick 无检查点 paused 分支在预算检查前先对账——批次已终态则按 workflowRunUpdateFromBatch 收敛运行状态（与 running 分支同款），下一轮走终态分支正常封存续跑 | tests/web/novel-store-flows.test.ts 新增「AN-062①：修订落库后 store chapters 缓存立即可见」（旧实现断言失败：缓存停留旧正文）；tests/web/cruise.test.ts 新增「AN-062②：无检查点 paused 运行 + 批次已终态 → 先对账收敛，不空转 dispatch」（断言首轮收敛 completed、零 dispatch、次轮照常封存开下一周期——旧实现三项全失败）；pnpm check 260 项全绿 |
...
| AN-052 | [x] | 封存检查与候选落库竞态，新批次候选漏审即冲线 | 2026-09-13 实测：每周期都新建批次并跑完（AN-051 生效），但 AN-047 检查在生成中途执行时看不到候选——检查完候选才落库，封存直接冲线，61 章以后字数未变。修复：封存前对「任务更新时间晚于本轮运行启动」的章节，校验其最新候选必须为 accepted，否则继续等待自动接受 | pnpm check 254 项通过 |
| AN-051 | [x] | 重放周期复用旧完成批次导致本轮空转冲线 | 2026-09-13 实测：重放 51–60 时 resumeGeneration 无 batchId 会走 createGenerationDraft，但该周期已有一个昨晚的 completed 批次——旧任务全部终态、候选早已接受，本轮直接封存冲线，一个新字都没写。修复：run.workflow 无自有批次时一律新建（批次与运行同生同死，不复用历史批次）| pnpm check 254 项通过；重放场景回归由 AN-047/049 测试覆盖任务与候选口径，批次口径由本修复的语义保证 |
| AN-050 | [x] | 封存前残留正史建议阻断封存，tick 静默空转 | 2026-09-13 实测：41–50 周期候选全部接受后，2 条早候选的正史建议晚于自动接受第二步产生，封存门禁拒绝且错误被 tick 吞掉，巡航 3ms 空转不动。修复：completed 分支封存前遍历本批次任务的候选，pending 正史建议全部自动接受（授权语义与自动接受一致，单条失败不阻断、门禁兜底）| 复刻封存链路验证 savePlanningCycle 成功；pnpm check 254 项通过 |
| AN-049 | [x] | 重写场景自动接受命中旧候选，新稿永远无法入正史 | 2026-09-13 实测：重写已封存周期时任务在新候选生成前仍指向上一轮 accepted 旧候选；自动接受按 candidateId 接受旧稿「成功」，42–50 章 2000+ 字新候选全部悬空（章节字数不变）。修复：①自动接受接受前检查同章是否存在更新的 candidate，有则先 updateGenerationJob 改指新稿再接受；②listChapterCandidates 按创建时间倒序，保证「最新候选」语义确定 | tests/web/auto-review.test.ts AN-049 回归：旧候选指向下改指新稿并接受新稿、不碰旧稿；pnpm check 254 项通过 |
| AN-048 | [ ] | Web 平台镜像装载失败不重试，novels 落空表导致批次误报「作品不存在」 | 2026-09-12/13 两次实测：loadNovels 偶发读空后 store.novels 落成空数组且不再重试，runBatch 的 buildContext 找不到作品即抛「Novel or chapter not found」，批次失败（数据无损，删批重启后自愈）| loadNovels/listX 装载结果为空但源数据非空时重试一次并告警；或镜像读取失败抛错而非落空表。回归：模拟首读失败→二读成功，批次正常生成 |
