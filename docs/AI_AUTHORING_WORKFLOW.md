...
| 项 | 值 |
| --- | --- |
| 已完本 | 5 本（各 120/120 章入正史、12/12 周期封存）：①《泊星港》巡航版（mock，242,437 字）；②《灯匠与雾海》（mock，达标版）；③《万纹忍尊》（真实 GLM-5.2，251,365 字，§7.4）；④《泊星港（真实重制版）》（真实 GLM-5.2，260,293 字，§7.5，novelId `AHnIo0LCrnLUaKkNnvQee`）；⑤《灯匠与雾海（真实重制版）》（真实 GLM-5.2，262,030 字，§7.6，novelId `k7qgJ6HZ6YzgCpqfpPVfC`） |
| 书稿 | `书稿/*-全120章*.md`（最新：`灯匠与雾海-真实GLM5.2-全120章.md`） |
...
### 7.6 《灯匠与雾海（真实重制版）》从零建书完本记录（2026-09-25）

**novelId `k7qgJ6HZ6YzgCpqfpPVfC`，真实 GLM-5.2 全程，自 createNovel 起 5.3 小时
跑完 120/120 章、12/12 周期封存，巡航自动收工。**

- 成果：262,030 字、单章最短 1808（终检后第 18 章补写至 1823，零低于 1800 门）、
  120 标题全唯一、长文（≥15 字）段首跨章查重仅 1 处（115/117 章指环物件呼应，
  风格性保留）、正史英文残留 0（20 处已清：5 章 `<entry>` 草稿包裹块、7 处
  E-XXX 内部编号引用、1 处 salt 夹中文等）。
- 与 mock 版（§7.1 表②）同设定重制：premise 按原 NEW_BOOK_BIBLE/BRIEF 重写
  （守灯人苏小灯、以记忆为灯油的不灭之灯、九十九座沉镇、雾议会非纯粹反派、
  「灯是记忆，雾是遗忘」主轴），正文为真实模型全新生成。
- 值守处置实录（§8.3 套餐）：①第 7 章 AI 审查非法 JSON 致批次 failed →
  原批 `retryFailedJobs` 复活续跑（比重派省 6 章 token）；②resumeCruise 后
  巡航把已入正史的 1–10 章整段重派（AN-061 盲区再现）→ 冻结批次后逐任务改指
