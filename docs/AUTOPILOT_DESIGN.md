...
> 可执行流程、门禁与真实评估标准见 `AUTOPILOT_WORKFLOW.md`。
...
| AI 生成分卷 + **全部章节标题** + 章纲 | ✅ 已实现并增加完整章数门禁 |
| 逐章批量生成、暂停续跑、候选稿 | ✅ 已实现 |
| AI 审查、事实提取、正史回写 | ✅ 已实现；未采纳候选禁止写正史 |
| 人物分层（主角/配角/酱油/龙套） | ✅ 已实现基础数据、规划与上下文过滤 |
| 题材名称库与查重 | ✅ 已实现基础池、实体/别名查重和已用名登记 |
| 场景卡与场景复用 | ✅ 已实现基础字段、视觉锚点和按章纲注入 |
| 限流退避、硬 Token 预算、候选链 | ✅ 已实现基础可靠性闭环 |
| 十步规划向导与人工检查点 | ✅ 已实现（Amy 逐项补问、确认后解锁） |
| 规划检查点持久化与下游失效 | ✅ `planning_workflows`，Web/Electron 同契约并随项目备份 |
| 10–20 章真实模型评估 | ⏳ 待作者提供模型配置并确认费用 |
