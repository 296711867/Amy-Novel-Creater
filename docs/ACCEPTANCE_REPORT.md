...
## 2026-09-02 AN-035（二）巡航精化：跳过重复地基、向导跟随与正名

**用户实测反馈**：巡航完成 31–40 后收工（目标 40），手动点「确认记忆回写，
规划下一批」发现人工流程要逐步点击；并确认希望 7–9 步纳入巡航、页面跟随
当前步骤。

**分析与修复**：

- 7–9 步本就在巡航循环体内（结构规划 → 提案自动处理 → 总检代签 → 建批
  → 自动审阅）；用户经历的是"到站下车"后的人工流程边界；
- **修浪费**：巡航开下一周期时原实现从 bible 全流程重跑（每周期多 3 次
  圣经/人物/场景模型调用）。`startWorkflowRun` 新增 `startPhase` 参数，
  第 2 周期起（起始章 > 1）直接从 structure 启动——回归断言
  generateNovelPlan 仅 ["structure"]；
- **向导跟随**：巡航运行期间规划页步骤条自动跳到当前阶段
  （structure→第 7 步、generation→第 10 步），暂停/停止后停止跟随；
- **正名**：卡片标题改为「全自动巡航（自动规划 + 自动创作）」，说明写明
  循环含 7–9 步且地基不重跑。

**验证**：`tests/web/cruise.test.ts` 9 项（新增/强化断言：起始新周期与封存
续开下一周期均只跑结构规划）；`pnpm check` 216 项测试通过。

## 2026-09-02 AN-035 巡航实战加固：六个线上停摆的定位与修复

用户开启巡航跑 31–40 章期间连续反馈停摆，全部以只读数据库取证定位：

1. **重复新增设定提案**（36b0057）：31-40 规划产出「新增：还魂灯（青灯）
   模块图纸」，该设定已由正文建议入正史，AN-005 门禁拒绝重复建档 → 巡航
   重试 3 次暂停。修复：提案自动接受逐条容错，"已存在"= 意图已满足 →
   自动拒绝跳过并注明"重复新增跳过 N"；
2. **检查点运行策划包代确认**（bd588df）：巡航接管的运行是 checkpoint
   模式，策划包停在 plan_review 须作者确认才可建批次 → 建批次被范围门禁
   拒绝且每 3 秒重试导致界面闪烁。修复：failed 且错误含"策划包"时代确认
   续跑；phase_review 续跑前预确认；
3. **批次终态主动对账**（c6344c5）：批次完成事件丢失时运行状态不收敛，
   巡航卡在"确认记忆回写"等人。修复：running 分支每轮对账，批次终态即
   收敛；封存后若下一周期已有策划包（手动规划）则复用直接进正文；
4. **chapter_review 秒回死洞**（7227fcf）：残留 paused/chapter_review 的
   运行 + 已完成批次，syncWorkflowRunFromBatch 因 running 前置空返回 →
   每轮静默空转、文案永停"正在检查"。修复：直接按批次终态更新运行；
5. **单轮看门狗**（1155194）：任一 IPC 挂起不返回时防重入锁永久占死
   （停止/重启无效）。修复：每轮 390s 看门狗超时解锁并明示，代际标记防
   误删新轮锁，超时不计失败；
6. **已人工封存周期**（d614302）：作者手动"确认记忆回写"封存过 31-40
   后，巡航封存环节报"未找到本周期策划包"暂停。修复：completed 视为已
   封存，跳过封存直接进下一周期。

另：巡航卡片样式对齐（.workflow-card label 边距泄漏）与间距统一、"从当前
进度续写"文案（显示已入正史 X 章最新第 N 章、输入下限=当前进度）、
chapter_review 进度数字显示（"第 X–Y 章已入正史 N/M"）、开启文案注明首轮
可能含 1–5 分钟模型调用。

**验证**：`tests/web/cruise.test.ts` 12 项（循环各分支 + 重复提案跳过 +
策划包自愈 + 已封存跳过 + 看门狗 + chapter_review 收敛）；`pnpm check`
218 项测试通过；AN-013（一）双端构建全部 chunk <500KB。真实长跑（41-50
及后续周期巡航至 85）效果待用户实测。

## 2026-09-02 AN-035 巡航实战加固（二）：七个线上问题与周期记忆清理

继上午六个停摆修复后，用户实测 41–70 章期间再定位修复七个问题（全部只读
取证 + 回归测试）：

1. 章级进度显示（44932d5）：正文阶段卡片显示"第 X–Y 章已入正史 N/M +
   批次 tokens"，替代静态阶段名；
2. 悬空状态自动清理 + 伏笔年龄修正（542bf44）：悬空引用触发 AN-027 全局
   门拦停自动接受 → 封存与自动接受恢复两处自动清理孤儿状态后复查；伏笔
   超期年龄改按已入正史章节计算（原按目录 85 章算，第 48 章新埋伏笔被误
   报"已 37 章未回收"污染规划上下文）；
3. 自动审阅看门狗（9ba7222）：补齐与巡航同款的挂起解锁（60s，代际标记）；
4. 收尾模式（43982bd）：无门禁批次连续生成全部候选后批次即 completed，
   自动审阅按"无活动批次"提前收工留下最后一章无人接受（49/10 卡住）→
   自停前检查最近完成批次是否仍有 candidate_ready，有则继续处理；
5. 进度数字每轮强制刷新（65723a2）：原用 store 缓存仅空时加载，入正史后
   卡片冻结在 0/10；
6. 陈旧缓存防误伤（f704474）：候选解析原优先内存缓存，缓存停留在
   "candidate"而库中已 accepted → 反复 accept 被拒（主进程日志刷屏
   Candidate already reviewed）→ 连续失败自停 → 巡航每轮重拉形成互搏，
   51–60 冻结在 5/10 且封存不执行 → 改为每轮以数据库为准；
7. 周期记忆清理（ba958d6）：封存时例行清理四类确定性垃圾——悬空状态、
   同人物同章同文重复状态、同名同埋设章重复伏笔、同章同名同文重复时间
   线（各留最早一条，计数入状态条）；语义层面的伪伏笔废弃仍归作者。
   背景：用户记忆库膨胀至 648 条伏笔、437 条时间线，第 61 章上下文
   10.6 万 tokens。

巡航循环体全流程（含记忆清理与全局门禁语义）文档化到
`AUTOPILOT_WORKFLOW.md` §6.3。验证：巡航 13 项 / 自动审阅 13 项 /
域与全量 `pnpm check` 222 项测试通过；41–70 章真实数据上巡航端到端
连跑成功（60 章入正史时 61–70 自动接续）。

## 2026-09-02（晚）主进程 SQLite 写锁挂起事件与恢复验证（AN-036 立项）

61–70 章全部入正史后，封存与后续所有写入停摆 ≥30 分钟：外部只读查询正常、
事件/封存/记忆清理零落库、巡航看门狗解锁重试仍撞在挂起的写入上。重启应用
后巡航自动断点恢复，2 分钟内完成：61–70 封存 → 周期记忆清理（伏笔
740 → 175，删除 565 条重复/垃圾）→ 71–80 结构规划启动。判定为主进程
SQLite 写锁挂起（具体持有者未取证），已按 AN-036 登记根治任务；全部实战
问题（13 项已修 + 1 项待根治）汇编为 `CRUISE_FIELD_NOTES.md` 现场手册。

## 2026-09-04 AN-022 状态复核与文档收口

- 双端 TypeScript 类型检查通过；
- AN-022 编排、状态映射、SQLite 持久化与巡航联动定向测试：31/31 通过；
- `pnpm verify`：46 个测试文件通过、2 个按条件跳过；241 项测试通过、
  11 项真实 API/环境相关测试按条件跳过；
- Web 与 Electron 两套生产构建通过，所有前端 chunk 均小于 500 kB；
- 路线图中 P3.2、P3.3、P3.6 及 AN-010/011 发布门槛的过期状态已与
  `BACKLOG.md` 对齐。

## 2026-09-04 全自动安全链与分章 ZIP 验收（AN-039/040）

- 全自动策略双端统一为 AI 章节审查；本地检查与 AI findings 共同参与重写；
- 无人值守正文 90% 字数门生效；AI 审查、事实提取及全局校验异常均 fail-closed；
- 巡航/自动审阅看门狗不再强制解锁并发执行；周期封存按更新时间选人物最新状态；
- 批次预算耗尽后，「继续巡航」为当前批次追加 60,000 输出 token 并断点续跑；
- 导出页新增分章文本 ZIP：作品信息、设定、每章独立 TXT，只收已入正史章节；
- `pnpm verify` 双端构建通过；最终 `pnpm check`：48 个测试文件通过、2 个按条件跳过；247 项测试通过、
  11 项真实 API/环境相关测试按条件跳过；Web 与 Electron 生产构建均通过。

## 2026-09-12 巡航无人值守缺陷收口与 120 章完本实跑（AN-042/043/044）

- AN-042（巡航与自动审阅批次收工竞态）：`tickCruise` 顶部识别"本批次已全部完成"
  的正常自停并重新联动自动接受，其余停用原因仍 fail-closed 暂停。
  `tests/web/cruise.test.ts` 新增双向回归（正常自停不暂停且照常封存续跑 /
  质量门停用仍暂停），全部通过；
- AN-043（巡航暂停后卡片不可达）：巡航启用（含 paused）期间巡航卡片在向导任意
  步骤可见，Autopilot 卡片保持第 10 步原行为。
  `tests/web/cruise-card-visibility.test.tsx` 两项挂载断言通过；
- AN-044（Web 端批次中断黑洞章）：`runBatch` 入口把孤儿任务
  （generating/building_context）重置回 queued 重新生成。线上实证：被刷新中断的
  第 37 章自愈重生成入正史；Web 宿主 `runBatch` 入口的自动化测试欠账已在
  BACKLOG 注记（现有测试骨架均为 Electron 宿主）；
- 实跑验收（Web 端 + 本地作者模型，零 API 费用）：同日《泊星港（巡航版）》
  21–120 章续跑完本——120/120 章入正史、12/12 周期封存、巡航自动收工，
  总字数 66,380；正史记忆 80 实体 / 40 时间线 / 33 伏笔 / 19 角色状态；
  完本备份 `backups/boxport-cruise-20260912-complete-120chapters.json`；
- `pnpm check`：49 个测试文件通过、2 个按条件跳过；251 项测试通过、
  11 项真实 API/环境相关测试按条件跳过；双端 TypeScript 类型检查通过。

## 2026-09-13 全书字数达标改造与重放链路收口（AN-046～053）

- 修复重放已封存周期的六个缺陷（详见 BACKLOG AN-046/047/049/050/051/052）
  与一个产品能力缺口（AN-053 clearWorkflowRuns，双端实现）；
- 作者模型替身升级为长度自适应（按提示词目标字数确定性扩写），替身内容
  仅用于链路验证，不代表文学质量；
- 实跑验收：120/120 章经软件自动链路全部达标——全书 242,437 字，单章
  最短 1816 字，零低于 1800（番茄上架 1000 字硬线之上、2000 推荐线附近）；
  12/12 周期封存，版本快照 477 个；
- 书稿《书稿/泊星港-巡航版-全120章-达标版.md》；备份
  backups/boxport-cruise-20260913-standard-120chapters.json；
- pnpm check：49 个测试文件通过、2 个按条件跳过；254 项测试通过、
  11 项真实 API/环境相关测试按条件跳过；双端 TypeScript 类型检查通过。

## 2026-09-22 《泊星港（真实重制版）》真实 API 续跑完本（10/120 → 120/120）

- 续跑自 2026-09-20 干净暂停点（周期 1 封存，10/120），真实智谱 GLM-5.2
  （Coding Plan）全程生成，值守式自动化（§8 手册照本宣科 + §8.3 恢复套餐实战）；
- **120/120 章入正史、12/12 周期封存、巡航自动收工**（巡航记录自动移除）；
- 终检四项：全书 **260,293 字**、单章最短 1903（零低于 1800）；标题 120/120
  唯一；长文（≥15 字）段首跨章查重仅 1 处（77/78 章交界悬崖回声句，风格性
  保留，符合手册甄别标准）；正史英文残留 0（8 处已清，E-XXX 实体编号除外）；
- 值守处置：页面重载 6 次自愈（桥接/Key/心跳重挂 + 空壳批次取消）、质量门
  5 次（4 次带修订要求重写、1 次改计划对齐正文、1 次文本修复、1 次视角段
  改写）、实体类型错位 2 次修正、47 份重载重派悬空稿拒绝清理——完整实录
  见 BACKLOG 2026-09-22 段；
- 产物：`书稿/泊星港-真实GLM5.2-全120章.md`（262,758 字符，UTF-8 校验）、
  `backups/泊星港-真实GLM5.2.amy-novel.json`（2.9MB，JSON.parse 校验通过，
  120 章 / 282 候选）；
- `pnpm check` 结果见本次提交前运行记录。
