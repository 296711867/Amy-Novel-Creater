...
- **Autopilot 自动运行**：第 10 步选定检查点档位（关键节点暂停 / 每章暂停 / 全自动候选）后，由 Workflow Runner 自动推进规划与当前批次正文；待审提案与候选稿仍按正史门禁等待作者处理，暂停、失败与完成均可断点恢复，已通过真实模型全流程评估。另有「全自动巡航」：一个按钮跨周期连跑到目标章数——设定提案与正文候选自动接受、周期自动封存、自动规划下一卷；出错 / 批次预算耗尽 / 校验异常自动暂停并记录原因，处理后一键续跑，重启应用自动接续。2026-09 起，巡航支持「重放」：对已写完的区间可解封周期后一键重写（字数门按作品当前单章标准执行），运行索引自动清理，详见 docs/CRUISE_FIELD_NOTES.md。
...
- **生成任务队列**：批次串行执行、候选链承接（后章读前章候选）、Token 硬预算、429/5xx 退避重试、暂停恢复与集中审阅；生成状态跨页面保持，切页回来按钮仍在转；已完成/已取消的历史批次记录可随时删除清理（已入正史的章节不受影响）。
- **自动接受连续创作**：作者可显式委托审阅——开启后每章候选稿与全部正史建议自动接受、自动继续写下一章直到本批完成；error 级检查问题、预算耗尽或连续失败会自动停下并交还人工，默认关闭、随时可停。开关持久化，刷新/重启后断点续跑；圣经外新出场角色自动建立最小档案卡；批次状态（生成中/暂停/排队）实时显示在状态条；同一作品同时只允许一个未完结批次。
- **全局一致性审查**：章节与事实建议合并入正史后，零 token 的确定性校验器随时检查引用悬空、位置跳变、道具重复持有、伏笔超期与别名冲突（error 级发现会阻断封存与自动接受）；每周期可再发起一次 AI 语义审查全局对照人物、道具与剧情，产出可忽略的发现清单与走人工审核的设定修复提案，并支持「按反馈重写第 N 章」把审查意见注入重写上下文。
- **故事总览与全书审读**：连续性页「故事总览」一屏看全人物出场、道具流转、伏笔进度与各章摘要（纯本地聚合零 token），并附记忆体检——重复状态一键合并、伏笔批量废弃/删除；整书连读页支持目录跳转连读全部正史、随手标记疑点入审查台账，以及全书分窗口 AI 通读审稿（滚动摘要续读，抓跨章矛盾与文风漂移）；「全局修订」可全书查找替换，逐章预览确认、每次替换留版本快照可回滚。
