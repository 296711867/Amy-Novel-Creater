...
- **Autopilot 自动运行**：第 10 步选定检查点档位（关键节点暂停 / 每章暂停 / 全自动候选）后，由 Workflow Runner 自动推进规划与当前批次正文；待审提案与候选稿仍按正史门禁等待作者处理，暂停、失败与完成均可断点恢复，已通过真实模型全流程评估。另有「全自动巡航」：一个按钮跨周期连跑到目标章数——设定提案与正文候选自动接受、周期自动封存、自动规划下一卷；出错 / 批次预算耗尽 / 校验异常自动暂停并记录原因，处理后一键续跑，重启应用自动接续。
